...
- 依赖的第三方库存在漏洞，固定成了没有漏洞的版本 (by @Darkatse)

## 4.9.1

### 🐛修复

- TauriTavern 兼容功能导致旧版本酒馆不能使用酒馆助手的问题
...
- 当脚本名字中包含某些字符时, `启用兜底清理`实验功能无效的问题
- 兼容使用了 pinia 4.0.0+ 的脚本/前端，例如 mvu 在 2026/07/15 因 pinia 更新无法生效

## 4.8.18

### 🗣提示词查看器

- 现在提示词查看器支持查看工具调用消息 (by @zxcv1050135042)

## 4.8.17

### 📦函数

- 现在 `injectPrompts` 注入的提示词会在脚本卸载时自动取消注入
...
- 新增`启用兜底清理`实验选项 (by @lynhi): 一些脚本在酒馆页面上附加了新的界面如悬浮球, 但是关掉脚本时没有清理它; 这个选项开启后, 酒馆助手会尝试清理这些界面
