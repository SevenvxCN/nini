/**
 * st-regex-drag-optimizer
 *
 * 目标：
 * - 优化 Regex 脚本面板（全局/预设/局部）拖拽排序卡顿
 * - 不改酒馆源代码，仅通过前端扩展 JS/CSS 改善拖拽体验
 *
 * 背景：
 * - 酒馆核心在 regex 扩展中使用 jQuery UI sortable 对以下容器启用拖拽排序：
 *   - #saved_regex_scripts
 *   - #saved_preset_scripts
 *   - #saved_scoped_scripts
 * - stop 回调会根据 DOM 顺序重建脚本数组并保存，然后 reloadCurrentChat/loadRegexScripts。
 *
 * 核心思路（与 st-worldinfo-drag-optimizer 一致）：
 * - 默认使用“指示线拖拽”：拖动过程中不让列表项重排，只移动轻量拖拽影子 + 指示线；松手时一次性移动 DOM 并调用 core 的 stop 保存逻辑。
 *   - 好处：大幅减少拖拽过程中的 layout/paint（条目 DOM 越复杂越明显）
 * - 兼容模式：仍使用 jQuery UI sortable，但提供轻量 helper/placeholder（不改 stop 逻辑）。
 */

import { registerCocktailSubpanel } from '../core/subpanels.js';

const EXTENSION_NAME = 'st-regex-drag-optimizer';
...
const _ALREADY_LOADED = Boolean(globalThis.__stRegexDragOptimizerLoaded);
if (_ALREADY_LOADED) {
  console.debug(`[${EXTENSION_NAME}] already loaded, skipping init`);
} else {
  globalThis.__stRegexDragOptimizerLoaded = true;
}
...
function applyBodyClasses() {
  ...
  body.classList.toggle('st-rdo-enabled', enabled);
  body.classList.toggle('st-rdo-engine-indicator', enabled && _settings?.engine === 'indicator');
  body.classList.toggle('st-rdo-engine-sortable', enabled && _settings?.engine === 'sortable');
}
...
function getRegexLists() {
  ...
  const out = [];
  for (const id of ['saved_regex_scripts', 'saved_preset_scripts', 'saved_scoped_scripts']) {
...
function pickScriptTitle(scriptLabelEl) {
  if (!(scriptLabelEl instanceof Element)) return '';

  const nameEl = scriptLabelEl.querySelector('.regex_script_name');
  const name = String(nameEl?.textContent || '').trim();
  if (name) return name;

  const t = String(scriptLabelEl.getAttribute('title') || '').trim();
  if (t) return t;

  const id = scriptLabelEl.getAttribute('id') || scriptLabelEl.dataset?.id;
  if (id) return `脚本 ${id}`;
...
function makeLiteHelper($, itemEl) {
  const labelEl = itemEl?.closest?.('.regex-script-label') || itemEl;
  const title = pickScriptTitle(labelEl);

  const helper = document.createElement('div');
  helper.className = 'st-rdo-helper';
...
function patchSortableOptionsForRegexList(listEl) {
  ...
  try {
    ...
    $list.sortable('option', {
      ...
      placeholder: 'st-rdo-placeholder',
      cursor: 'grabbing',
      scroll: true,
...
function createGhost(draggedEl, widthPx) {
  ...
  ghost.className = 'st-rdo-helper st-rdo-drag-ghost';
  ghost.textContent = pickScriptTitle(draggedEl);
...
function createIndicator(widthPx, leftPx, topPx, zIndex) {
  ...
  el.className = 'st-rdo-drop-indicator';
...
function getRegexItemAtPoint(listEl, draggedEl, x, y) {
  let el;
  try { el = document.elementFromPoint(x, y); } catch { el = null; }
  if (!(el instanceof Element)) return null;

  const item = el.closest('.regex-script-label');
  if (!(item instanceof HTMLElement)) return null;
  if (item === draggedEl) return null;
  if (item.closest(`#${CSS.escape(listEl.id)}`) !== listEl) return null;
  return item;
}

function getFirstLastItem(listEl, draggedEl) {
  const all = Array
    .from(listEl.querySelectorAll('.regex-script-label'))
...
function getPointerXY(e) {
  if (e && typeof e === 'object') {
    ...
    if (changed && changed.length) return { x: changed[0].clientX, y: changed[0].clientY };
  }
  return { x: /** @type {any} */ (e).clientX, y: /** @type {any} */ (e).clientY };
}

function updateDropTarget(d) {
  // Always keep rect up to date (wheel scroll may move the settings panel).
  ...
  const hitItem = getRegexItemAtPoint(d.listEl, d.draggedEl, x, y);
  if (hitItem) {
  ...
  // If we didn't hit an item (e.g. over whitespace), fall back:
  const { first, last } = getFirstLastItem(d.listEl, d.draggedEl);
  if (!first || !last) {
    // empty list, place indicator at list top
    d.refEl = null;
    d.insertBefore = true;
  ...
  // Inside list but not on an item: keep last known target (prevents jitter).
...
function endDrag({ commit }) {
  ...
  // Commit DOM reorder once, then reuse core stop callback to persist.
  try {
    const ref = d.refEl;
...
function startIndicatorDrag(startEvent, listEl, draggedEl) {
  ...
  logDebug('indicator drag start', {
    title: pickScriptTitle(draggedEl),
    list: listEl?.id,
...
function installIndicatorEngine(listEl) {
  ...
  const onPointerDown = (e) => {
    ...
    const draggedEl = handle.closest('.regex-script-label');
    ...
    if (e.type === 'mousedown' && typeof /** @type {any} */ (e).button === 'number' && /** @type {any} */ (e).button !== 0) return;
  ...
  logDebug('indicator engine installed', { list: listEl.id });
  return true;
}
...
function refreshRuntime() {
  ...
  const lists = getRegexLists();
  if (!lists.length) return;

  // Install the selected engine for each list.
  for (const { el } of lists) {
    if (_settings?.engine === 'indicator') {
      installIndicatorEngine(el);
    } else {
      patchSortableOptionsForRegexList(el);
    }
  }
}
...
function installObserver() {
  ...
  try {
    ...
    const root = document.getElementById('extensions_settings')
      ...
      || document.body;
    _observer.observe(root, { subtree: true, childList: true, attributes: true, attributeFilter: ['class', 'style'] });
  } catch (e) {
  ...
  root.innerHTML = `
    <div class="cocktail-grid">
      <label class="cocktail-check">
        <input id="st_rdo_enabled" type="checkbox">
      ...
      <label class="cocktail-field" title="推荐：指示线拖拽（拖动中不重排条目，松手一次性移动并保存），对重 UI 条目更流畅。">
        ...
        <select id="st_rdo_engine" class="text_pole">
      ...
      <label class="cocktail-field" title="仅对“原生 sortable（跟随移动）”引擎生效：lite=轻量拖拽块；clone=克隆原条目（更接近原始外观，但更重）。">
        ...
        <select id="st_rdo_helper" class="text_pole">
      ...
      <label class="cocktail-check" title="仅对“原生 sortable”生效：把拖拽 helper 挂到 body，避免被列表 overflow 裁剪。">
        <input id="st_rdo_appendToBody" type="checkbox">
        helper 追加到 body（避免裁剪）
      </label>

      <label class="cocktail-check" title="拖拽接近列表上下边缘时自动滚动，便于把脚本拖到很远的位置。">
        <input id="st_rdo_autoScroll" type="checkbox">
        边缘自动滚动
      </label>

      <label class="cocktail-field" title="指针距离列表上下边缘小于该值时开始自动滚动。">
        <span class="cocktail-label">滚动触发距离(px)</span>
        <input id="st_rdo_scrollEdge" type="number" min="0" max="300" step="1">
      </label>

      <label class="cocktail-field" title="拖拽靠近边缘时的最小滚动速度（每帧像素）。">
        <span class="cocktail-label">最小速度(px/f)</span>
        <input id="st_rdo_scrollMin" type="number" min="0" max="80" step="0.5">
      </label>

      <label class="cocktail-field" title="拖拽贴近/越过边缘时的最大滚动速度（每帧像素）。">
        <span class="cocktail-label">最大速度(px/f)</span>
        <input id="st_rdo_scrollMax" type="number" min="0" max="120" step="0.5">
      </label>

      <label class="cocktail-check">
        <input id="st_rdo_debug" type="checkbox">
        Debug log
      </label>
    </div>
    ...
    <div class="cocktail-help">
      <div>说明：</div>
      <div>- 核心 regex 扩展使用 jQuery UI sortable 对脚本列表排序，stop 时会保存顺序并刷新聊天。</div>
      <div>- 本模块默认把拖拽过程改为“指示线拖拽”：拖动中不触发列表重排，只在松手时一次性移动并复用 core stop 回调保存。</div>
      <div>- 如遇兼容性问题，可切换到“原生 sortable”。</div>
    </div>
  `;
  container.appendChild(root);

  const $enabled = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_enabled'));
  const $engine = /** @type {HTMLSelectElement|null} */ (root.querySelector('#st_rdo_engine'));
  const $helper = /** @type {HTMLSelectElement|null} */ (root.querySelector('#st_rdo_helper'));
  const $appendToBody = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_appendToBody'));
  const $autoScroll = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_autoScroll'));
  const $scrollEdge = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_scrollEdge'));
  const $scrollMin = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_scrollMin'));
  const $scrollMax = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_scrollMax'));
  const $debug = /** @type {HTMLInputElement|null} */ (root.querySelector('#st_rdo_debug'));
